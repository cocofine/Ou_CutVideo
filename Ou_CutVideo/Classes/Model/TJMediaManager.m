...
+ (void)addBackgroundMiusicWithVideoUrlStr:(NSURL *)videoUrl audioUrl:(NSURL *)audioUrl andCaptureVideoWithRange:(TimeRange)videoRange completion:(void(^)(NSString *strPath,AVAssetExportSession *assetExportSession))completionHandle {
    ...
    NSString *strPath = [NSString stringWithFormat:@"%ld.mov",(long)[[NSDate date] timeIntervalSince1970]];
    NSString *outPutPath = [NSTemporaryDirectory() stringByAppendingPathComponent:strPath];
...
//    NSLog(@"^^^^^^^^^%ld",[self degressFromVideoFileWithURL:outPutUrl]);
    ...
    [assetExportSession exportAsynchronouslyWithCompletionHandler:^{
        ...
        completionHandle(strPath,assetExportSession);
    }];
}
